...
export async function getExamResults(schoolId: string | null, options: {
  ...
  const transformed: ExamResultData[] = resultsList.map((result) => ({
    ...
    avgPercentage: (result as any).overallPercentage || result.percentage.overallPercentage || result.percentage || 0,
  }));
...
export async function getAnalytics(schoolId: string | null): Promise<AnalyticsData> {
  ...
  examResults.forEach((result) => {
    ...
    const score = (result as any).overallPercentage || result.percentage.overallPercentage || result.percentage || 0;
    if (existing) {
  ...
  for (const result of examResults) {
    ...
    if (grade) {
      ...
      gradeGroups.get(grade)!.push((result as any).overallPercentage || result.percentage.overallPercentage || result.percentage || 0);
    }
  }
