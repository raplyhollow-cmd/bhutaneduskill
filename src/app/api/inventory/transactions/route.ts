...
export async function POST_bulkIssue(request: NextRequest) {
  try {
    const authResult = await requireAuth(["admin"]);
    if ("error" in authResult) {
      return NextResponse.json(
    ...
    const data: BulkIssueInput = await request.json();

    if (!data.items || data.items.length === 0) {
      return NextResponse.json(
        { error: "No items provided for issue", status: 400 } satisfies ApiErrorResponse,
        { status: 400 }
      );
    }
    ...
    const now = new Date();
    const transactionDate = now.toISOString().split("T")[0];
    const results: typeof inventoryTransactions.$inferInsert[] = [];
    ...
    for (const itemRequest of data.items) {
      try {
        // Get current item
        const item = await db.query.inventoryItems.findFirst({
          where: eq(inventoryItems.id, itemRequest.itemId),
        });

        if (!item) {
          errors.push({ itemId: itemRequest.itemId, error: "Item not found" });
          continue;
        }

        // Check stock
        const newQuantity = item.quantity - itemRequest.quantity;
        if (newQuantity < 0) {
          errors.push({
            itemId: itemRequest.itemId,
            error: `Insufficient stock. Current: ${item.quantity}, Requested: ${itemRequest.quantity}`,
          });
          continue;
        }
        ...
        const [newTransaction] = await db
          ...
          .values({
            ...
            destinationLocation: `Issued to: ${data.issuedToName}`,
            performedBy: userId,
            reason: itemRequest.reason || data.notes || "Bulk issue",
            createdAt: now,
          })
          .returning();
        ...
        await db
          ...
          .set({
            quantity: newQuantity,
            assignedTo: data.issuedTo,
            assignedDate: transactionDate,
    ...
    logger.info("Bulk issue completed", {
      userId,
    ...
    return NextResponse.json({
      ...
      message: `Bulk issue completed. ${results.length} items issued, ${errors.length} failed.`,
    } satisfies ApiSuccess<{
      transactions: typeof results;
      errors: typeof errors;
    }>);
  } catch (error) {
    logger.apiError(error, { route: "/api/inventory/transactions", method: "POST_bulkIssue" });
    return NextResponse.json(
      { error: "Failed to complete bulk issue", status: 500 } satisfies ApiErrorResponse,
      { status: 500 }
    );
  }
}
...
// ============================================================================
// POST /api/inventory/transactions/bulk-return - Bulk return items
// ============================================================================
...
export async function POST_bulkReturn(request: NextRequest) {
  try {
    const authResult = await requireAuth(["admin"]);
    if ("error" in authResult) {
      return NextResponse.json(
    ...
    const data: BulkReturnInput = await request.json();

    if (!data.items || data.items.length === 0) {
      return NextResponse.json(
        { error: "No items provided for return", status: 400 } satisfies ApiErrorResponse,
        { status: 400 }
      );
    }
      ...
      try {
        ...
        const [newTransaction] = await db
          ...
          .values({
            ...
            quantity: itemRequest.quantity,
            balanceAfter: newQuantity,
            sourceLocation: `Returned by: ${data.returnedByName}`,
            destinationLocation: item.location || null,
            performedBy: userId,
            reason: itemRequest.notes || `Returned by ${data.returnedByName}`,
            createdAt: now,
          })
          .returning();
        ...
        await db
          ...
          .set({
            quantity: newQuantity,
            condition: itemRequest.condition || item.condition,
            assignedTo: null,
            assignedDate: null,
            assignedUntil: null,
            status: "available",
    ...
    return NextResponse.json({
      ...
      message: `Bulk return completed. ${results.length} items returned, ${errors.length} failed.`,
    } satisfies ApiSuccess<{
      transactions: typeof results;
      errors: typeof errors;
    }>);
  } catch (error) {
    logger.apiError(error, { route: "/api/inventory/transactions", method: "POST_bulkReturn" });
    return NextResponse.json(
      { error: "Failed to complete bulk return", status: 500 } satisfies ApiErrorResponse,
      { status: 500 }
    );
  }
}
