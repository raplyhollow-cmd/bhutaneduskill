...
// ============================================================================
// POST /api/inventory/alerts/check-low-stock - Check and create low stock alerts
// ============================================================================

export async function POST_checkLowStock(request: NextRequest) {
  try {
    const authResult = await requireAuth(["admin"]);
    if ("error" in authResult) {
      return NextResponse.json(
    ...
    // Get all categories for alert thresholds
    ...
    const categoryThresholdMap = new Map(
      categories.map((cat) => [cat.id, cat.alertThreshold || 10])
    );

    // Get all items
    const items = await db.query.inventoryItems.findMany({
      where: sql`${inventoryItems.schoolId} = ${user.schoolId || ""}`,
    });

    const now = new Date();
    const newAlerts: typeof inventoryAlerts.$inferInsert[] = [];

    for (const item of items) {
      const minimumStock = item.minimumStock ?? categoryThresholdMap.get(item.categoryId) ?? 10;

      if (item.quantity <= minimumStock) {
        // Check if active alert already exists
        const existingAlert = await db.query.inventoryAlerts.findFirst({
          where: and(
            eq(inventoryAlerts.itemId, item.id),
        ...
        if (!existingAlert) {
          const severity = item.quantity <= minimumStock / 2 ? "critical" : "warning";
          ...
          newAlerts.push({
            ...
            message: `Item "${item.name}" is running low on stock. Current: ${item.quantity}, Minimum: ${minimumStock}`,
            status: "active",
    ...
    if (newAlerts.length > 0) {
      await db.insert(inventoryAlerts).values(newAlerts);
    }

    logger.info("Low stock check completed", { userId, newAlerts: newAlerts.length });
    ...
    return NextResponse.json({
      ...
      message: `Low stock check completed. ${newAlerts.length} new alerts created.`,
    } satisfies ApiSuccess<{
      newAlerts: number;
      alerts: typeof newAlerts;
    }>);
  } catch (error) {
    logger.apiError(error, { route: "/api/inventory/alerts", method: "POST_checkLowStock" });
    return NextResponse.json(
      { error: "Failed to check low stock", status: 500 } satisfies ApiErrorResponse,
      { status: 500 }
    );
  }
}
