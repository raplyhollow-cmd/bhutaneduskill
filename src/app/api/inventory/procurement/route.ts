...
export async function POST_receive(request: NextRequest) {
  try {
    const authResult = await requireAuth(["admin"]);
    if ("error" in authResult) {
      return NextResponse.json(
    ...
    const data: ReceiveItemsInput = await request.json();

    if (!data.purchaseOrderId || !data.items || data.items.length === 0) {
      return NextResponse.json(
        { error: "Missing required fields: purchaseOrderId, items", status: 400 } satisfies ApiErrorResponse,
        { status: 400 }
      );
    }
    ...
    const now = new Date();
    const receivedDate = data.receivedDate || now.toISOString().split("T")[0];
        ...
        if (existingItem) {
          ...
          await db
            ...
            .set({
              ...
              condition: item.condition || existingItem.condition,
          ...
          return NextResponse.json(
            { error: `Item not found: ${item.itemId}`, status: 404 } satisfies ApiErrorResponse,
            { status: 404 }
          );
        }
    ...
    if (successCount === data.items.length) {
      await db
        .update(purchaseOrders)
        .set({
          status: "received",
    ...
    logger.info("Received items from purchase order", {
      userId,
    ...
    return NextResponse.json({
      ...
      message: `Items received successfully. ${successCount}/${data.items.length} items updated.`,
    } satisfies ApiSuccess<{
      results: typeof results;
      successCount: number;
    }>);
  } catch (error) {
    logger.apiError(error, { route: "/api/inventory/procurement", method: "POST_receive" });
    return NextResponse.json(
      { error: "Failed to receive items", status: 500 } satisfies ApiErrorResponse,
      { status: 500 }
    );
  }
}
