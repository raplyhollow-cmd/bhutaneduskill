...
import { users, parents, parentToStudent, attendance, homeworkSubmissions, assessments, examResultsEnhanced, feePayments, careerMatches, studentFees } from "@/lib/db/schema";
...
export const GET = createApiRoute(
  async (request: NextRequest) => {
    ...
    try {
      // FERPA COMPLIANCE: Get children via parent_to_student join table
      ...
      logger.apiError(error, { route: "/api/parent/dashboard", method: "GET" });
      return errorResponse("Failed to fetch parent dashboard data", 500);
    }
  },
  ['parent']
);
