/**
 * FEE GENERATOR PAGE (Platform Admin View)
 *
 * Allows platform admins to generate annual/termly fee invoices
...
const FEE_TYPE_OPTIONS = [
  { value: "sdf", label: "SDF", description: "School Development Fund" },
  { value: "rimdro", label: "Rimdro", description: "Annual Prayer/Blessing Fee" },
  { value: "diary", label: "Diary", description: "School Diary & ID Card" },
  { value: "sports", label: "Sports", description: "Sports Equipment Fee" },
  { value: "stationery", label: "Stationery", description: "Stationery Fee" },
  { value: "tuition", label: "Tuition", description: "Tuition Fee" },
  { value: "lab", label: "Lab", description: "Laboratory Fee" },
  { value: "library", label: "Library", description: "Library Fee" },
  { value: "transport", label: "Transport", description: "Transport Fee" },
  { value: "uniform", label: "Uniform", description: "Uniform Fee" },
];
...
export default function FeeGeneratorPage() {
  ...
  const fetchSchoolData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/schools/${schoolId}/generate-fees`);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <PremiumCard>
            <CardHeader>
              ...
              <CardDescription>
                Configure the academic session and fee structure
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  The academic year for these fees (e.g., "2026" for 2026 session)
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Leave empty for default: {isGovernment ? "February 15" : "February 15 / July 15"}
                </p>
              </div>
            </CardContent>
          </PremiumCard>
            ...
            <CardContent className="space-y-4">
              {feeStatus?.session.year ? (
                <>
                  ...
                  {feeStatus.session.generationDate && (
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Last Generated</span>
                      <span className="text-sm">{new Date(feeStatus.session.generationDate).toLocaleDateString()}</span>
                    </div>
                  )}
                  <div className="pt-3 border-t space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Total Generated</span>
                      <span className="font-semibold">{feeStatus.fees.totalGenerated}</span>
                    </div>
                    <div className="flex items-center justify-between">
                ...
                <p className="text-center text-gray-500 py-4">
                  No fees generated for this school yet
                </p>
              )}
            </CardContent>
          </PremiumCard>
          ...
          <PremiumCard>
            ...
            <CardContent>
              ...
              <p className="text-center text-sm text-gray-500">
                Students will receive fee invoices
              </p>
            </CardContent>
          </PremiumCard>
        </div>
