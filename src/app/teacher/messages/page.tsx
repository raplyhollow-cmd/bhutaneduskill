 ...
 *
 * A clean, mobile-first chat interface for teachers to communicate with parents.
 *
 * Features:
 * - Thread list showing all conversations with parents
 * - Chat view for individual conversations
 * - Real-time message sending/replies
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="relative">
          <div className="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-semibold">
            {thread.parentImage ? (
              ...
              <img
                src={thread.parentImage}
                alt={thread.parentName}
                className="w-full h-full rounded-full object-cover"
              />
            ) : (
              <span>{thread.parentName.charAt(0)}</span>
            )}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <h3 className="font-semibold text-gray-900 truncate">{thread.parentName}</h3>
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div
          ...
          style={{ borderColor: "rgb(59 130 246)", borderTopColor: "transparent" }}
        />
      </div>
    );
  }
          ...
          <CardContent className="p-0">
            {threads.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-600">
                  Parents can start conversations with you from their portal
                </p>
              </div>
            ) : (
              threads.map((thread) => (
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-3 flex-1">
          <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-semibold">
            {selectedThread?.parentName.charAt(0)}
          </div>
          <div>
            <h2 className="font-semibold text-gray-900">{selectedThread?.parentName}</h2>
