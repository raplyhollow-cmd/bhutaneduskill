...
export default function ParentMedicalPage() {
  ...
  useEffect(() => {
    if (selectedChild) {
      fetchMedicalData(selectedChild.id);
    }
  }, [selectedChild]);
  ...
  const fetchMedicalData = async (studentId: string) => {
    try {
      const res = await fetch(`/api/student/medical?studentId=${studentId}`);
      const data = await res.json();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            View your children's medical history and health information
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() => selectedChild && fetchMedicalData(selectedChild.id)}
                ...
                <SelectContent>
                  {children.map((child) => (
                    <SelectItem key={child.id} value={child.id}>
                      {child.name}
                      {child.classGrade && ` (Class ${child.classGrade}`}
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-gray-500">
              Unable to load child information. Please contact support.
            </p>
          </CardContent>
        </Card>
      ) : (
            ...
            <TabsContent value="visits">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Infirmary visits for {selectedChild.name}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-500">
                                  {record.visitType.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase())}
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-2 pt-2 border-t">
                              ...
                              <span className="text-sm text-green-600">
                                Discharged: {record.dischargeCondition.replace(/_/g, " ")}
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            <TabsContent value="vaccinations">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Immunization history for {selectedChild.name}
                  </CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <TabsContent value="allergies">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Known allergies, chronic conditions, and special medical needs for {selectedChild.name}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {!medicalData?.allergies || medicalData.allergies.length === 0 ? (
                              ...
                              <div>
                                {allergy.allergenName ? (
                                  <>
                                    ...
                                    <p className="text-sm text-gray-500">
                                      {allergy.allergenType.replace(/_/g, " ")} Allergy
                                    </p>
                                  </>
                                ) : allergy.conditionType ? (
                                  <>
                                    <p className="font-semibold text-gray-900">
                                      {allergy.conditionType.replace(/_/g, " ")}
                                    </p>
