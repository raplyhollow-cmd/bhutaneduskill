/**
 * BHUTAN STANDARD SUBJECTS SEED - WORKING VERSION V4
 */
...
async function seedBhutanSubjects() {
  ...
  for (const subject of bhutanSubjects) {
    for (const grade of subject.grades) {
      ...
      try {
        // Cast string to JSON type using ::jsonb
        await sql.query(
          ...
          [
            ...
            code,
            subject.type,
            JSON.stringify(subject.type), // subject_type is json
            `${subject.name} for Grade ${grade}`,
            grade,
            JSON.stringify(subject.grades), // applicable_grades is json
            true,
          ]
        );
