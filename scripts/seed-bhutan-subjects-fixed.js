/**
 * BHUTAN STANDARD SUBJECTS SEED - FIXED VERSION
 *
 * Standard subjects for Bhutan middle schools (Grades 6-12)
 *
 * Design Principle:
 * - Subject codes are GLOBALLY unique across all schools (e.g., MATH-9)
 * - This enables cross-school comparison and national analytics
 * - Each school can reference the same subject code
 * - Subject metadata (name, type, description) is shared
 * - But schools can have different assignments/teachers per subject
 */
...
async function seedBhutanSubjects() {
  ...
  // Get the first school (we'll create subjects for it)
  ...
  console.log(`🏫 Using school: ${school.code} (${school.id})\n`);

  let createdCount = 0;
  let skippedCount = 0;

  // First, create subjects without school_id (global subjects)
  // These will be the "templates" that all schools can reference
  console.log("=== Creating Global Subject Templates ===\n");
  ...
  for (const subject of bhutanSubjects) {
    for (const grade of subject.grades) {
      ...
      try {
        // Insert with null school_id to create global template
        await sql`
        ...
        if (check.length > 0) {
          ...
          console.log(`  ⏭️  Skipped: ${subject.name} Grade ${grade} (${code}) - already exists`);
          skippedCount++;
        }
      } catch (err) {
        console.log(`  ❌ Error: ${subject.name} Grade ${grade} - ${err.message}`);
      }
    }
  }
