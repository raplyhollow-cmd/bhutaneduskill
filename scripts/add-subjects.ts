  ...
  for (const school of schools) {
    ...
    for (const subject of bhutanSubjects) {
      for (const grade of subject.grades) {
        ...
        // Use template string with neon - it supports ${} interpolation
        try {
          await sql`
          ...
          `;
          console.log("  ✅ " + subject.name + " Grade " + grade);
          created++;
        } catch (err: any) {
          console.log("  ⏭️  Skip: " + subject.name + " Grade " + grade);
        }
      }
    }
  }
